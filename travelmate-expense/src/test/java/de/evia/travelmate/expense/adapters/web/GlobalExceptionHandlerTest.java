...
class GlobalExceptionHandlerTest {
    ...
    @Test
    void responseStatusNotFoundReturns404() throws Exception {
        mockMvc.perform(get("/test-errors/response-status-not-found"))
            .andExpect(status().isNotFound())
            .andExpect(view().name("error/404"))
    ...
    @Test
    void responseStatusForbiddenReturns403() throws Exception {
        mockMvc.perform(get("/test-errors/response-status-forbidden"))
            .andExpect(status().isForbidden())
            .andExpect(view().name("error/error"))
            .andExpect(model().attribute("message", "Authentication required"));
    }

    @Test
    void responseStatusNotFoundWithHtmxTriggersToast() throws Exception {
        mockMvc.perform(get("/test-errors/response-status-not-found")
                .header("HX-Request", "true"))
            .andExpect(status().isNotFound())
            .andExpect(view().name("fragments/empty :: empty"))
    ...
    static class TestErrorController {
        ...
        public String notFound() {
            throw new EntityNotFoundException("Expense", "abc-123");
        }

        @GetMapping("/duplicate")
        public String duplicate() {
            throw new DuplicateEntityException("expense.error.alreadyExists");
        }

        @GetMapping("/business-rule")
        public String businessRule() {
            throw new BusinessRuleViolationException("expense.error.alreadySettled");
        }
        ...
        public String responseStatusNotFound() {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Trip not found");
        }

        @GetMapping("/response-status-forbidden")
